...
import { Lock, Ruler, Scale, Activity, TrendingUp, Zap } from 'lucide-react';
            ...
            <div className="flex items-center gap-4 bg-gray-900/50 p-4 rounded-3xl border border-gray-800">
                ...
                <div className="flex-grow">
                    ...
                    <div className="flex gap-2 mt-2">
                        ...
                        <span className="text-[10px] bg-black border border-gray-700 px-3 py-1 rounded-full text-gray-300 font-bold uppercase">{profile.age || '?'} YRS</span>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <div className="flex flex-col gap-3">
                    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-3 flex-1 flex flex-col justify-center relative overflow-hidden">
                        <p className="text-[10px] font-bold uppercase text-gray-500 flex items-center gap-1 relative z-10"><Ruler size={10}/> Height</p>
                        <p className="text-xl font-black text-white relative z-10">{currentHeight} <span className="text-[10px] text-gray-500">cm</span></p>
                    </div>
                    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-3 flex-1 flex flex-col justify-center relative overflow-hidden">
                        <p className="text-[10px] font-bold uppercase text-gray-500 flex items-center gap-1 relative z-10"><Activity size={10}/> Body Fat</p>
                        <p className="text-xl font-black text-cyan-400 relative z-10">{currentFat}<span className="text-[10px] text-gray-500">%</span></p>
                    </div>
                </div>
            </div>
            ...
            {currentWeight !== "--" && currentHeight !== "--" && (
                <div className="grid grid-cols-2 gap-3">
                    <div className="p-4 bg-gray-900/50 rounded-2xl border border-gray-800">
                        <span className="text-[10px] text-gray-500 font-bold uppercase block mb-1">BMI Score</span>
                        <span className="font-mono text-xl font-black text-white">{calculateBMI(currentWeight, currentHeight)}</span>
                        <p className="text-[9px] text-gray-500 uppercase mt-1 opacity-70">General Health</p>
                    </div>
                    <div className="p-4 bg-gray-900/50 rounded-2xl border border-gray-800 relative overflow-hidden">
                        <div className="absolute right-0 top-0 p-2 opacity-10"><Zap size={40} className="text-yellow-500"/></div>
                        <span className="text-[10px] text-gray-500 font-bold uppercase block mb-1">FFMI Score</span>
                        <span className="font-mono text-xl font-black text-yellow-400">{calculateFFMI()}</span>
                        <p className="text-[9px] text-gray-500 uppercase mt-1 opacity-70">Muscle Potential</p>
                    </div>
                </div>
            )}
                    ...
                    p.weight ? (
                        <div key={i} className="flex items-center justify-between py-3 px-4 bg-gray-900 rounded-2xl border border-gray-800/50">
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full bg-black flex items-center justify-center text-gray-400 font-bold text-[10px] border border-gray-800">
                                    {new Date(p.date).getDate()}
                                </div>
                                <div>
                                    <p className="text-xs font-bold text-white">Weigh In</p>
                                    <p className="text-[10px] text-gray-500">{new Date(p.date).toLocaleDateString(undefined, {month:'short', year:'numeric'})}</p>
                                </div>
                            </div>
