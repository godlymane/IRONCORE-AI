...
export function useFitnessData() {
  ...
  const [data, setData] = useState({ 
      ...
      profile: { xp: 0, coins: 0, inventory: [] }, 
      leaderboard: [], chat: [], following: [], posts: [], inbox: [],
      globalFeed: [], battles: [] // New Battle State
  });
  ...
  const login = async () => {
    if (!auth) return;
    try { await signInWithPopup(auth, new GoogleAuthProvider()); } catch (e) { alert("Login failed: " + e.message); }
  };
  ...
  useEffect(() => {
    ...
    const bind = (key, path, isDoc = false) => {
        ...
        listeners.push(onSnapshot(actualRef, (snap) => {
            if (isDoc) { setData(prev => ({ ...prev, [key]: snap.exists() ? snap.data() : {} })); } 
            else { 
