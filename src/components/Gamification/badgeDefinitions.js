...
export const BADGES = [
  ...
  },
  {
    ...
    category: 'CONSISTENCY',
    rarity: 'common',
    check: (d) => (d.workoutCount || d.workouts?.length || 0) >= 10,
  },
  {
    id: 'sixty_day_forge',
    name: 'Sixty Day Forge',
    description: 'Maintain a 60-day Forge streak',
    icon: '🌟',
    category: 'CONSISTENCY',
    rarity: 'epic',
    check: (d) => (d.longestForge ?? d.longestStreak ?? 0) >= 60,
  },
  {
    id: 'iron_will',
    name: 'Iron Will',
  ...
  // ─── STRENGTH ───
  {
    id: 'iron_score_rising',
    name: 'Rising',
    description: 'Reach an Iron Score of 30',
    icon: '📈',
    category: 'STRENGTH',
    rarity: 'uncommon',
    check: (d) => (d.ironScore || 0) >= 30,
  },
  {
    id: 'iron_score_forged',
    name: 'Forged',
    description: 'Reach an Iron Score of 60',
    icon: '🔥',
    category: 'STRENGTH',
    rarity: 'rare',
    check: (d) => (d.ironScore || 0) >= 60,
  },
  {
    id: 'iron_score_elite',
    name: 'Elite',
    description: 'Reach an Iron Score of 80',
    icon: '⚡',
    category: 'STRENGTH',
    rarity: 'epic',
    check: (d) => (d.ironScore || 0) >= 80,
  },
  {
    id: 'iron_starter',
    name: 'Iron Starter',
  ...
  },
  {
    ...
    category: 'SOCIAL',
    rarity: 'uncommon',
    check: (d) => !!(d.guildRole === 'leader' || d.ownedGuildId || d.earnedBadges?.includes('guild_leader')),
  },
  {
    id: 'squad_goals',
    name: 'Squad Goals',
    description: 'All guild members log a workout in the same week',
    icon: '🤝',
    category: 'SOCIAL',
    rarity: 'rare',
    check: (d) => !!(d.earnedBadges?.includes('squad_goals')),
  },
  {
    id: 'undefeated',
    name: 'Undefeated',
    ...
    category: 'SOCIAL',
    rarity: 'epic',
    check: (d) => (d.consecutiveWins ?? d.currentWinStreak ?? 0) >= 5 || !!(d.earnedBadges?.includes('undefeated')),
  },
  {
    id: 'gladiator',
    name: 'Gladiator',
  ...
  // ─── MASTERY ───
  {
    ...
    category: 'MASTERY',
    rarity: 'common',
    check: (d) => (d.progressCount || d.progress?.length || 0) >= 1,
  },
  {
    id: 'on_track',
    name: 'On Track',
    ...
    category: 'MASTERY',
    rarity: 'uncommon',
    check: (d) => (d.progressCount || d.progress?.length || 0) >= 10,
  },
  {
    id: 'transformation',
    name: 'Transformation',
    description: 'Log weight 50 times — committed to the journey',
    icon: '🦋',
    category: 'MASTERY',
    rarity: 'rare',
    check: (d) => (d.progressCount || d.progress?.length || 0) >= 50,
  },
  {
    id: 'nutritionist',
    name: 'Nutritionist',
